...
$(function() {
    var topoffset=43; //the hight of the nav bar
...
.click(function(event) {
  ...
  if (
    ...
    if (target.length) {
      ...
      $('html, body').animate({
        scrollTop: target.offset().top-topoffset // adding the offset of the nav bar 
      }, 1000, function() {
...
$(window).scroll(function() {
  ...
  topoffset; // for the navigation bar hight 
  $('nav li a').removeClass('active'); // while scrolling the page remove the active class 

  if (windowpos > $('#hotelinfo').offset().top) //if we pass the element 
  {
    $('nav li a').removeClass('active'); //clear all active classes
    $('a[href$="#hotelinfo"]').addClass('active'); //adding active class 
  } //windowpos

  if (windowpos > $('#rooms').offset().top) //if we pass the element 
  {
    $('nav li a').removeClass('active'); //clear all active classes
    $('a[href$="#rooms"]').addClass('active'); //adding active class 
  } //windowpos

  if (windowpos > $('#dining').offset().top) //if we pass the element 
  {
    $('nav li a').removeClass('active'); //clear all active classes
    $('a[href$="#dining"]').addClass('active'); //adding active class 
  } //windowpos

  if (windowpos > $('#events').offset().top) //if we pass the element 
  {
    $('nav li a').removeClass('active'); //clear all active classes
    $('a[href$="#events"]').addClass('active'); //adding active class 
  } //windowpos

  if (windowpos > $('#attractions').offset().top) //if we pass the element 
  {
    $('nav li a').removeClass('active'); //clear all active classes
    $('a[href$="#attractions"]').addClass('active'); //adding active class 
  } //windowpos
}); //(window).scroll
    ...
    var controller = new ScrollMagic({
        globalSceneOptions: {
            triggerHook: "onLeave"
        } // we trigger the event (starting an animation) when the element leave the screen  
    }); //ScrollMagic

    var pin = new ScrollScene ({
        triggerElement: '#nav'   
    }).setPin('#nav').addTo(controller); //ScrollScene pin navigation

    var attractionstween = TweenMax.staggerFromTo('#attractions article', 1, {opacity: 0, scale: 0}, // the affect of acticle element start with opacity 0 & scale 0 
    {delay: 1, opacity:1, scale:1, // the affect of acticle element start with 1 sec delay and ends with opacity 1 & scale 1
      ease: Back.easeOut}); // the effect, for more details: https://greensock.com/docs/Easing/Back

    // create the scene that is going to play the animation 
    var scene = new ScrollScene({   
        triggerElement: '#attractions',
        offset: -topoffset-1 //trigger this animation when we pin the nav bar 
